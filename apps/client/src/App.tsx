...
function ClientMiniApp() {
  ...
  useEffect(() => {
    if (!token || savedCityVerified || cashLocation?.status !== "PENDING" || !calculation?.calculation_token) return;
    return startVisiblePolling(async (signal) => {
      ...
      const next = await withAuth((authToken) => persistentCityEnabled
        ? api.getCashLocationChallenge(authToken, cashLocation.id, signal)
        : api.createCashLocationChallenge(authToken, {
        calculation_token: calculation.calculation_token,
        send_prompt: false,
      }), token);
      setCashLocation(next);
      if (persistentCityEnabled && next.status === "VERIFIED") {
  ...
  async function confirmCashLocation(throughBot = false) {
    ...
    setError("");
    try {
      if (persistentCityEnabled && !throughBot) {
  ...
  async function submitOrder() {
    ...
    try {
      const calc = calculation || (await calculate());
      if (!calc) throw new Error("EMPTY_CART");
      if (paymentMethod === "cash" && cashLocationRequired && !savedCityVerified && cashLocation?.status !== "VERIFIED") {
  ...
  const content =
    route.name === "dish" ? (
      ...
      activeOrder && !allowConcurrentDevOrders ? <ActiveOrderLock order={activeOrder} locale={locale} /> : <Checkout
        ...
        locationBotRequested={locationBotRequested}
        onOpenLocationBot={() => confirmCashLocation(true)}
...
function checkoutCopy(locale: Locale) {
  const copy = {
    ru: {
      ...
      locationDefaultTitle: "Проверим зону доставки",
      ...
      locationDefaultText: "Убедимся, что вы в Нови-Саде и сможем привезти заказ. Точные координаты не сохраняем.",
    },
    sr: {
      ...
      locationDefaultTitle: "Provera zone dostave",
      ...
      locationDefaultText: "Proverićemo da ste u Novom Sadu i da možemo da dostavimo porudžbinu. Tačne koordinate ne čuvamo.",
    },
    en: {
      ...
      locationDefaultTitle: "Check delivery area",
      ...
      locationDefaultText: "We’ll check that you’re in Novi Sad and that we can deliver your order. We don’t store your exact coordinates.",
    },
	...
	return (
		<div className="page narrow booking-page">
			...
			<section className="booking-guests" aria-label={copy.guestLabel}>
				...
				<div className="booking-guest-options" role="radiogroup" aria-label={copy.guestLabel}>
					{[1, 2, 3, 4, 5].map((count) => (
						<button
							type="button"
							role="radio"
...
function bookingCopy(locale: Locale) {
	if (locale === "sr") return {
		...
		guestLabel: "Broj gostiju", guestsLabel: "Gosti", timeLabel: "Vreme", chooseDate: "Izaberite datum", chooseTime: "Slobodno vreme", guests: (count: number) => `${count} ${count === 1 ? "gost" : "gostiju"}`, noSlots: "Nema mesta", empty: "Nema slobodnih termina u narednih sedam dana.",
	...
	if (locale === "en") return {
		...
		guestLabel: "Number of guests", guestsLabel: "Guests", timeLabel: "Time", chooseDate: "Choose a date", chooseTime: "Available times", guests: (count: number) => `${count} ${count === 1 ? "guest" : "guests"}`, noSlots: "Full", empty: "No available times in the next seven days.",
	...
	return {
		...
		guestLabel: "Количество гостей", guestsLabel: "Гости", timeLabel: "Время", chooseDate: "Выберите дату", chooseTime: "Свободное время", guests: (count: number) => `${count} ${count === 1 ? "гость" : count < 5 ? "гостя" : "гостей"}`, noSlots: "Мест нет", empty: "На ближайшие семь дней свободного времени нет.",
