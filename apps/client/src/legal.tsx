...
const legalCopy: Record<Locale, LegalCopy> = {
  ...
  en: {
    ...
    privacy: {
      ...
      sections: [
        { id: "data", title: "1. Data we process", bullets: ["Telegram identifier and public profile data needed for authentication and linking an order to the customer.", "Text address, comment, language and order history; phone number only if separately provided through legacy contact or support.", "Order contents and values, delivery status, selected payment method and status, complaints and support messages.", "Technical data needed for security and operation, such as request time, IP address and security events. Telegram initData is verified by the server and is not written to application logs.", "For cash-order location verification, exact coordinates are not retained; only the result, distance, accuracy, time and rejection reason are stored.", "Application usage data: screens opened and buttons or links clicked. Field contents, addresses, coordinates, phone numbers and Telegram initData are not included in analytics events."] },
        ...
        { id: "retention", title: "4. Retention and security", paragraphs: ["Data is kept only as long as needed: business and fiscal records for statutory periods, complaint records for at least two years, product analytics events for no more than 400 days, and security data for a limited period needed to protect the service and investigate incidents. Data is then deleted or anonymised unless another legal basis requires retention."], bullets: ["Addresses and optional legacy phone numbers are encrypted, staff access is role-limited, and sensitive values are not written to application logs.", "The cart stored on the device contains product IDs, display data and quantities only; it contains no address, phone, session or Telegram initData."] },
