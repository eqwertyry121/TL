  ...
  return {
    ...
    async analyticsCSV(_token, range) {
      ...
      const rows = [
        ...
        "",
        "retention_days,eligible_users,returned_users,percent",
        ...analytics.product.retention.map((row) => `${row.days},${row.eligible_users},${row.returned_users},${row.percent}`),
        "",
        "screen,views,unique_users",
        ...analytics.product.screens.map((row) => `${row.key},${row.events},${row.unique_users}`),
        "",
        "click_target,clicks,unique_users",
        ...analytics.product.clicks.map((row) => `${row.key},${row.events},${row.unique_users}`),
      ];
  ...
  const items: AdminMenuItem[] = [
    ...
    itemSeed(
      ...
      "12 × beef khinkali • 12 × cheese khinkali • 1 × Megrelian khachapuri • 1 × grape Natakhtari 1 L",
        "6282", "на 4–5 человек", 1, 10, media("full-house-card"),
    ),
    itemSeed(
...
function calculateAnalytics(range: AnalyticsRange): AdminAnalytics {
  ...
  return {
    ...
    product: {
      retention: [1, 7, 30].map((days) => ({ days, eligible_users: 0, returned_users: 0, percent: 0 })),
      screens: [],
      clicks: [],
      order_funnel: [
        { key: "app_open", events: demoVisitors, unique_users: demoVisitors },
        { key: "menu", events: demoVisitors, unique_users: demoVisitors },
        { key: "cart", events: orders.length, unique_users: orders.length },
        { key: "checkout", events: orders.length, unique_users: orders.length },
        { key: "order_created", events: orders.length, unique_users: orders.length },
      ],
      booking_funnel: [
        { key: "app_open", events: demoVisitors, unique_users: demoVisitors },
        { key: "booking", events: 0, unique_users: 0 },
        { key: "reservation_created", events: 0, unique_users: 0 },
      ],
    },
    statuses,
    payments,
