...
- Один заметный фирменный сигнал только при появлении нового order ID: громкая,
  но мягкая трёхнотная мелодия повторяется дважды и завершается длинным
  аккордом; общая длительность около 3,5 секунд. Одновременные события не
  накладывают несколько сигналов друг на друга.
