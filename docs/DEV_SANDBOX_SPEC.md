...
### Разовое подтверждение города (DEV, 2026-09-03)

- Только при `DEV_SANDBOX_MODE=true` сервер сохраняет `users.city_verified_at`.
...
- Production сохраняет прежнюю одноразовую проверку; sandbox-флаг не принимается
  из клиентских запросов. DEV и PROD продолжают использовать отдельные БД.
