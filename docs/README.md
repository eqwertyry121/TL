# Навигация по ТЗ

## Основные документы

1. [Мастер-ТЗ](00_MASTER_SPEC.md)
2. [Что было упрощено](SIMPLIFICATION_NOTES.md)
3. [Локальная разработка](local-development.md)
4. [OpenAPI](openapi.yaml)
5. [Domain architecture](domain-architecture.md)
6. [Backup and restore](backup-restore.md)
7. [Production alerts](production-alerts.md)
8. [UX-ТЗ переработки Admin Mini App](admin-ux-redesign.md)
9. [ТЗ глобальной оптимизации](GLOBAL_OPTIMIZATION_SPEC.md)
10. [Результаты performance-проверок](performance-results.md)
11. [Runbook глобальной оптимизации](optimization-runbook.md)
12. [Аудит завершённости глобальной оптимизации](optimization-completion-audit.md)

## Артефакты этапа 0

1. [Product config](product-config.md)
2. [Staff and bots](staff-and-bots.md)
3. [Payment/fiscal decisions](payment-fiscal-decisions.md)
4. [Menu template](menu-template.csv)
5. [Menu fixtures](menu-fixtures.csv)
6. [Minimal texts](i18n-minimal-texts.md)
7. [Wireframes](wireframes.md)
8. [GitHub Pages frontend hosting](github-pages.md)
9. [Open items](stage0-open-items.md)

## Этапы

1. [Этап 0 — исходные данные и контент](stages/00_INPUTS.md)
2. [Этап 1 — фундамент и backend](stages/01_FOUNDATION_BACKEND.md)
3. [Этап 2 — Client Mini App и cash-заказ](stages/02_CLIENT.md)
4. [Этап 3 — Kitchen и Courier](stages/03_STAFF.md)
5. [Этап 4 — Admin и простая аналитика](stages/04_ADMIN.md)
6. [Этап 5 — карты, крипто и фискализация](stages/05_PAYMENTS.md)
7. [Этап 6 — production, тестирование и запуск](stages/06_RELEASE.md)
8. [Этап 7 — бронирование столиков](stages/07_RESERVATIONS.md)

Будущий coding-агент выполняет только один этап за раз. Каждый этап заканчивается
работающим и проверяемым состоянием репозитория.
